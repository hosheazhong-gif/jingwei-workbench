...
# Local blank-topic walk. This is not a website. Does not overwrite jingwei-demo.sqlite3.
...
$DatabasePath = Join-Path $DatabaseDirectory "jingwei-walk.sqlite3"
$OutDirectory = Join-Path $DatabaseDirectory "walk"
New-Item -ItemType Directory -Force -Path $DatabaseDirectory | Out-Null
if (Test-Path $DatabasePath) {
    throw "Walk database already exists at $DatabasePath. Move or rename it before rerunning; the script will not overwrite it."
}

& $PythonCommand.Exe @($PythonCommand.Args) -m app.cli --db $DatabasePath demo-walk --out $OutDirectory --plain
if ($LASTEXITCODE -ne 0) { throw "demo-walk failed." }
