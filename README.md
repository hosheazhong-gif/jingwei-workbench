...
| **参与者盘点**（赛道扫描） | 名单本身就是要找的东西：这赛道有谁、最近冒出来哪几家 | ⚪ 只搭了骨架 |

**「验到什么程度」是认真的，不是装饰：**

- 🟢 **已在真题上走查过**——拿真实题目把整条循环从头走到尾跑过：拆问题、搜、扒原话、写稿、收下、导出。
- ⚪ **只搭了骨架，还没试过**——七条问法是照着公开做法拟的，每条都注明了来处，但一条都还没拿真题搜过。用之前先自己看一遍那七条。
...
**选哪个？别读定义，看句子。** 界面上打开「模板都是干什么的」，最上面是一张对照表——每个模板配一句典型的经理原话，哪句最像你手上这道题就点哪个：

> 「机器人手臂这个产业链缺什么环节？」→ 产业链分析
> 「把这三家的定价口径摆到一张纸上。」→ 竞品分析（定位与主张对标）
> 「这一季度对手都干了什么？」→ 竞争情报（定期深挖）
> 「目前 AIGC 有什么突出的新兴公司？」→ 参与者盘点（赛道扫描）

每个模板页里还有：三句例题、适用与不适用、流程图、分步骤（每步带「做完的标志」）、一个完整案例、常见的坑，以及**七条参考问法各自的来处**。

**最容易搞混的一对**：竞品分析假设你**已经知道跟谁比**；参与者盘点是**名单还不知道，要先找出来**。先盘点圈出名单，再换竞品分析逐家深挖。
...
打开后**没有弹窗**。要退出或打开数据目录，用工作台右上角的「退出经纬」「数据目录」。**关掉浏览器页面，经纬也会在几分钟内自己退出**，不留后台进程。

页面左上角「经纬」后面显示的是当前版本号——**想确认自己跑的是哪一版，看那儿**。
